...
describe("discord users routes", () => {
  it("GET /users/@me returns the bot user", async () => {
    ...
    const body = (await res.json()) as { id: string; bot: boolean; username: string };
  ...
  it("GET /users/:id returns a public user", async () => {
    ...
    expect(res.status).toBe(200);
    const body = (await res.json()) as { id: string; username: string; email?: string };
  ...
  it("PATCH /users/@me updates global_name and persists", async () => {
    ...
    expect(res.status).toBe(200);
    expect(((await res.json()) as { global_name: string }).global_name).toBe("Renamed Bot");
